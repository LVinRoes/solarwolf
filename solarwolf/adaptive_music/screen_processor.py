...
class ScreenProcessor:
    def __init__(self):
        ...
        self.capture_interval = 0.5  # in seconds
        self.last_intensity = 0.0

        # Attributes for the demo capture state machine
        self.demo_interval = 10  # cycle every 10 seconds
        ...
        self.demo_state = 'idle'  # possible states: 'idle', 'waiting_for_B'
    ...
    def capture_current_screen(self):
        """Captures the current screen and returns it as a NumPy array."""
        screen_surface = pygame.display.get_surface()
        if screen_surface is None:
            return None
        screen_array = pygame.surfarray.array3d(screen_surface)
    ...
    def downscale_and_grayscale(self, array):
        """Downscales the image (every 4th row/column) and converts it to grayscale."""
        ...
        if self.demo_state == 'idle':
            ...
            if current_time - self.screenshot_A_time >= 0.5:
                ...
                if screenshot_B is not None:
                    ...
                    print(f"Demo cycle {self.demo_cycle} completed.")
                    self.demo_cycle += 1
