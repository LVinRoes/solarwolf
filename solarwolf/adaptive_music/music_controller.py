...
from adaptive_music.melody_agent import MelodyAgent
from adaptive_music.rhythm_agent import RhythmAgent
from adaptive_music.bass_agent import BassAgent
from adaptive_music.harmony_helper import HarmonyHelper
from adaptive_music.harmony_agent import HarmonyAgent
    ...
    def debug_dump_precomputed_data(self) -> None:
        ...
        for level in range(1, 6):
            for instr_type in ["bass", "melody", "drums", "drone", "extra"]:
                ...
                logger.debug(f"  -> key={key} has {len(items_in_queue)} entries:")
...
class MusicController:
    def __init__(self) -> None:
        ...
        # Start buffer generation in separate threads
        self.start_buffer_generation()

    def setup_instruments(self) -> None:
        """
        Initializes the SCAMP instruments.
        """
    ...
    def generate_buffer_for_level(self, level: int) -> None:
        while not self.stop_flag:
            # Generate a progression and insert it into both queues simultaneously:
    ...
    def check_for_transposition(self) -> None:
        ...
        if time_in_high >= 16:
            logger.debug(f"4 bars at intensity {level} reached.")
    ...
    def run_scamp(self) -> None:
        ...
        print("LilyPond file 'output_score.ly' has been created.")
                ...
                for index, (old_instruments, dur) in enumerate(zip(pattern_pitches, pattern_durations)):
                    ...
                    if index >= num_subdivisions / 2:
                        ...
                        if abs(intensity_diff) > 0.5:
                            ...
                            print(f"Fill factor: {f}, intensity_diff: {intensity_diff}, reduced_factor: {reduced_factor}")
        ...
        while not self.stop_flag:
            ...
            if level < 4:
                ...
                if time_in_high >= 16:
                    ...
                    logger.debug(">>> Harmonies will be played." if do_harmony else ">>> No harmonies this time.")
                    self.high_intensity_start_time_harmony = self.session.beat()
    ...
    def _update_tempo_for_intensity(self, new_intensity: int, beats_for_transition: float = 8.0) -> None:
        ...
        logger.debug(f"Linear tempo transition from {old_bpm:.1f} to {new_bpm:.1f} BPM in {beats_for_transition} beats.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.debug("Music stopped by KeyboardInterrupt.")
