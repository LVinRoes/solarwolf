...
class GlobalInputAnalyzer:
    ...
    def on_press(self, key):
        ...
        print(f"[DEBUG] Key pressed: {key_name} (Total: {self.key_counts[key_name]})")

    def process_event(self, event):
        # Global inputs are captured via pynput – this method remains empty.
        pass
    ...
    def get_input_intensity(self):
        ...
        if elapsed_time >= self.reset_interval:
            ...
            if total_keys_pressed == 0:
                decay_factor = 0.5  # Decay factor; can be adjusted
                current_intensity = self.last_intensity * (1 - decay_factor)
            
            # Moving average (without exponential smoothing)
            ...
            print(f"[DEBUG] Average intensity: {average_intensity}")
