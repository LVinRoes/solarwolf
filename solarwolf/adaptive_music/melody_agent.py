...
class MelodyAgent:
    def __init__(self):
        # Original scale (A minor context, e.g. A, C, D, E, G)
    ...
    def _get_phrase_pattern(self, intensity_level=3):
        """
        Selects a phrase from the library, optionally applies a transposition or slight variation,
        and then converts it from A minor to C major.
        """
    ...
    def _assemble_4bar_phrase(self, intensity_level=3):
        """
        Generates a 4-bar phrase with a random structure:
         - A–A–B–A (original pattern)
         - A–B–A–C
         - A–B–C–D
        Each letter represents a phrase; identical letters mean the phrase is repeated.
        """
        # Randomly select a phrase pattern
        pattern_choice = random.choice(["A-A-B-A", "A-B-A-C", "A-B-C-D"])
        print(f"[DEBUG] Selected phrase pattern: {pattern_choice}")
    ...
    def generate_melody(self, intensity_level=3, total_duration=8.0, num_notes=16):
        ...
        print("[DEBUG] generate_melody (phrasing) for intensity:", intensity_level)
